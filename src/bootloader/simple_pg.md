...
前チャプターでは Ymir の ELF イメージをファイルシステムからメモリ上に読み込めるようになりました。
...
ページテーブルの構造を始めとして、ページテーブルの構造や操作は CPU アーキテクチャに大きく依存します。
本シリーズでは x86-64 以外をサポートしませんが、それでもアーキテクチャ依存のコードは階層を分けて書いていくことにします。
...
できる限りアーキ依存の概念を隠蔽できるような API を提供し、`arch/`以下のファイルが提供する API を直接参照できないようにします。
...
`arch/x86/arch.zig` はアーキ依存のコードの内 `/arch` 以上の階層から利用したいファイルを定義します。
...
これで `boot.zig` から x64 のページングに関する機能を参照できるようになりました。
...
返される物理アドレスは、エントリがページをマップする場合 (`.ps == true`) はマップするページの物理アドレスです。
ページテーブルを参照する場合 (`.ps == false`) は参照するページテーブルの物理アドレスになります。
...
自身が`Lv4Entry`ならば `LowerType` は `Lv3Entry` になります。
...
なお、この関数では対象の<u>仮想アドレスが既にマップされている場合は想定していません</u>。
...
[のちのチャプター](../kernel/page_allocator.md) で説明しますが、メモリタイプを `BootServicesData` にすることで
...
`Flags` から分かるとおり、どうやら **UEFI が提供する Lv4 テーブルは read-only になっている** ようです。
...
鶏が先か卵が先かみたいな問題になってしまいました。デッドロックです。
